...
def retrieve_results(model, qubits, depths, zne_extrapolator, mem, zne, stat, service: QiskitRuntimeService):
    mean_errors = []
    for qubit in qubits:
        for depth in depths:
            job_ids = retrieve_job_id(qubit, depth, zne_extrapolator, mem, f"logs/jobs_model_{model}_qubits_{qubits}_depths_{depths}_pauli-Z_weight_1_stat_shots_{stat}_zne_{zne}_mem_{mem}")
            res = []

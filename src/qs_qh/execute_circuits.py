...
def construct_logical_Pauli_observable(NUM_QUBITS: int, pauli_weight: int, type_pauli: str):
    paulis = ['I'*i + 'Z'*pauli_weight + 'I'*(NUM_QUBITS-i-pauli_weight) for i in range(NUM_QUBITS)]
    coeffs = 1/len(paulis)
...
def construct_batches_execution(LIST_QUBITS: list, LIST_DEPTHS: list, stat_shots: int=100, shots: int=4096, err_suppr: int=0, err_mitig: int=0, zne: bool=True, pauli_weight: int=1, type_pauli: str="Z"):    
    ...
    save_jobs(filename=f"logs/jobs_qubits_{LIST_QUBITS}_depths_{LIST_DEPTHS}_pauli-{type_pauli}_weight_{pauli_weight}_stat_shots_{stat_shots}_shots_{shots}_err_suppr_{err_suppr}_err_mitig_{err_mitig}_zne_{zne}", jobs=jobs_batches)
...
def construct_batch_execution(physical_circuit: QuantumCircuit, physical_observable: SparsePauliOp, backend: ibm_backend, options: EstimatorOptions, stat_shot: int, zne: bool):    
    ...
    for extrapolator, job_batch in jobs_batch.items():
        dict_batch = {"job_stat_number":stat_shot, "job_id":job_batch.job_id(), "extrapolator": extrapolator}

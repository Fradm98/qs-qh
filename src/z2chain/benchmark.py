...
def update_and_send_jobs(benchmarkdb, chain_length_arr, layers_arr, devices_arr, service, estimator_opt_dict, J, h, lamb, t_compute, x_basis=False):
    ...
    benchmarkdb.execute(chain_length_arr, layers_arr, devices_arr, service, z2_chain_benchmark_circuit, pauli_xs_mean if x_basis else pauli_zs_mean, estimator_opt_dict, test_circuit_name="z2_chain_trotter_compuncomp", observable_name="mean_paulis_" + ("x" if x_basis else "z"))
